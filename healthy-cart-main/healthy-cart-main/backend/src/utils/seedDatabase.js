...
import { seedSiteContent } from "../data/seedSiteContent.js";
...
export async function seedDatabase() {
  ...
  const missingProducts = seedProducts.filter((product) => !existingSlugs.has(product.slug));

  if (missingProducts.length > 0) {
    await Product.insertMany(missingProducts, { ordered: false });
  }

  const siteContent = await SiteContent.findOne({ key: seedSiteContent.key }).lean();
